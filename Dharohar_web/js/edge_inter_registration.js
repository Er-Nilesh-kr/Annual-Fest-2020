  ...
  if(ieee !=""){
   ...
   if(gender != ""){
     ...
     if(ieee == ""){
      ...
      $("#ieee").css("border","1px solid #ff8080");
      $("#ieee-error").text("Please select Yes / No").css("color","#ff8080").show();
     ...
     signInUser(ieee,name,college,course,branch,year,mobile,email,event,gender,rand,acc);
    }
  
   });
console.log("not error")

function signInUser(ieee,name,college,course,branch,year,mobile,email,event,gender,rand,acc)
{
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      if (user) {
        ...
        console.log("user is signed in")
          ...
          var uid = user.uid
        serviceMethod(ieee,name,college,course,branch,year,mobile,email,event,gender,rand,acc)
...
function serviceMethod(ieee,name,college,course,branch,year,mobile,email,event,gender,rand,acc) {
   
    var URef = firebase.database().ref().child("Edge_inter_registration_details").push();
    URef.set({
      REGISTER_Id:rand,
      IEEE_Member:ieee,
      Name: name,
 ...
 document.getElementById("ieee").value="";        
